...
import example.shared.lib.transactionTask.{ ReadTransaction, ReadWriteTransaction, TransactionTask }

class UserRepositoryImpl extends UserRepository with UserConverter {

  private val u = UserDataModel.syntax("u")

  override def resolveById[R: _task: _trantask](id: UserId): Eff[R, Option[User]] = {
//    val x = for {
//      tran <- ask[R, Transaction]
//      q <- {
//          Task.delay {
//      implicit val session: DBSession = fetchDBSession(tran)
//    fromTranTask(query.asInstanceOf[TranTask[Option[User]]])
//      }
//      res <- {
//        fromTranTask(ReadTransactionTask(q))
//      }
//    } yield res
//    x
  ...
  override def store[R: _task: _trantask](entity: User): Eff[R, User] = {
//    val x = for {
//      tran <- ask[R, Transaction]
//      q <- {
